...
int main() {
    ...
    out_file << "inf_prob,inf_period,theta,ptnr_duration,traders,r0_theory,r0_sim,equi_inf" << endl;
    ...
    double epsilon_traders;  //scaling constant for number of trading partners
    ...
    for (int np = 0; np < num_params; ++np) {
        ...
        //in ESM we only change one element of trade at a time, so comment out changes not wanted
        ...
        double new_traders = 0.1 + (0.1 * np);
        double traders_scalar = new_traders / k;
        epsilon_traders = new_traders * delta / (alpha * (N - 1 - new_traders));
        alpha *= epsilon_traders;
        b /= traders_scalar;    //in paper, so that in-flows are maintained, we change the number of trades when the
                                //number of trade partners is changed
        ...
        //if batch size and num trades are changed
        b /= epsilon_trade;
        theta *= epsilon_trade;
        //if partnership dynamics are changed
        alpha /= epsilon_ptnr;
        delta /= epsilon_ptnr;
        //if num trading partners is changed
        b *= traders_scalar;
        alpha /= epsilon_traders;
